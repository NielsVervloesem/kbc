...
def greengrass_infinite_infer_run():
    try:
        ...
        error, model_path = mo.optimize(model_name,input_width,input_height, aux_inputs={'--epoch': 5})
        print(model_path)
        ...
        while doInfer:
            ...
            #client.publish(topic=iotTopic, payload='stopk: ' + str(top_k))
            msg = '['
            prob_num = 0 
            for obj in top_k:
                #client.publish(topic=iotTopic, payload='probnum: ' + str(prob_num))
                if prob_num == topk-1: 
                    msg+= '{'
                    msg += '"id": {}, "prob": {:.2f}'.format(obj["label"]+1, obj["prob"]*100)
                    msg += '}'
                else:
                    msg+= '{'
                    msg += '"id": {}, "prob": {:.2f}'.format(obj["label"]+1, obj["prob"]*100)
